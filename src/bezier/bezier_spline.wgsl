...
fn sdBezier2(p: vec2<f32>, v0q: vec2<f32>, v1q: vec2<f32>, v2q: vec2<f32>) -> vec2<f32> {
    let i = v0q - v2q;
    let j = v2q - v1q;
    let k = v1q - v0q;
    let w = j - k;
    ...
    return vec2<f32>(length(v0 + t * (k + k + t * w)), 0.0) ;
}
...
fn tips(uv: vec2<f32>, m_in: vec4<f32>, dy: vec2<f32>, solid: f32, w: f32) -> vec4<f32> {
    var m = m_in;
    let theta = atan2(dy.y, dy.x) + 3.1415 / 2.0;
    let ma = mat2x2<f32>(vec2<f32>(cos(theta), -sin(theta)), vec2<f32>(sin(theta), cos(theta)));
    let uvrot = ma * uv ;
...
fn fragment(in: FragmentInput) -> @location(0)  vec4<f32> {
    ...
    // var out_col = vec4<f32>(0.2, 0.3, 0.8, 1.00);
    ...
    // let dc = sdCircle(in.uv, p1 + vec2<f32>(0.0, 6.0)  , 1.2);
    ...
    let d = sdRoundedBox(
        ...
        in.uv,
        bez_uni.inner_canvas_size_in_pixels / 2.0 - 1.0,
        vec4<f32>(r, r, r, r)
    );
    ...
    // out_col =  vec4<f32>(0.0,0.3,0.3,1.0);
